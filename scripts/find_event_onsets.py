...
def find_local_minima(x, y):
    ...
    for i in range(1, len(y) - 1):  # Exclude the endpoints because they can't be local minima
...
def find_local_maxima(x, y):
    ...
    if len(x) != len(y):
        raise ValueError("Arrays must have the same length")
    for i in range(1, len(y) - 1):  # Exclude the endpoints because they can't be local maxima
        if y[i] > y[i - 1] and y[i] > y[i + 1]:
...
def get_abs_normed_velocity(position_data):
    ...
    if np.max(vels) > 0:  # Should almost always be the case
        vels /= np.max(vels)  # Normalize so each value is weighted the same
    ...
    if grasp_pairs is not None:
        ...
        for pt in grasp_pairs:
            ...
            if 'label' in kwargs.keys():
                del kwargs['label']  # So we don't relabel the grasp events
...
def main(server, sessions, trials_sel, temp, overwrite,
    ...
    """
    Main script function that creates logs, aggregates trials, and finds timepoints for each trial.

    Args:
        server (str): The server directory where trial data is stored.
        sessions (list of str): A list of session directories to process.
        (empty list means all trials).
        temp (str): The path to the temporary directory for log files.
        overwrite (bool): Whether to overwrite existing output files (if True) or skip them
        (if False).
        processes (int): The number of processes to use for parallel execution.
        make_plots (bool): Whether to create timepoint plots in each session directory.

    Returns:
        None

    Raises:
        ValueError: If the server directory does not exist or is inaccessible.
    ...
    >>> num_processes = 4
    >>> create_plots = True
    >>> main(server_directory, session_directories, temp_dir, overwrite_output,
...
if __name__ == "__main__":
    ...
    # main(args.server, args.sessions, args.temp, args.overwrite, args.processes)
    main(
        args.server,
        args.sessions,
        args.trials,
