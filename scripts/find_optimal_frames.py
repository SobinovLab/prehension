...
def calculate_optimal_time(trial, mdof, optimal_frames, i_trial):
    column_names, values = ncams.utils.import_csv(trial.post_kinematic_filename_csv)
...
def main(server, sessions, trials_sel, temp, processes, overwrite):
    ...
    for session in tqdm.tqdm(sessions, ncols=100, desc='Sessions'):
        ...
        # run the pool
        if len(p_args) > 0:
            pool = ReportingPool(calculate_optimal_time, p_args, processes=processes,
...
if __name__ == '__main__':
    ...
    main(args.server, args.sessions, args.trials, args.temp, args.processes, args.overwrite)
