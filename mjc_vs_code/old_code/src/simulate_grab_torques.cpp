...
//----------------------------- MY
const std::vector<std::string> DOF_MAP_OPENSIM = {
    ...
    "elbow_flexion",
    "ra_el_e_f", "ra_wr_e_f",
    "ra_cmc1_ad_ab", "ra_cmc1_f_e", "ra_mcp1_f_e",
    "ra_mcp2_e_f", "ra_mcp2_ab_ad", "ra_dip2_e_f",
    "ra_mcp3_e_f", "ra_pip3_e_f", "ra_dip3_e_f",
    "ra_mcp4_e_f", "ra_pip4_e_f", "ra_dip4_e_f",
    "ra_mcp5_e_f", "ra_pip5_e_f"};

const std::vector<std::string> DOF_MAP_MUJOCO = {
    "", "",
    "", "",
    "", "", "",
    "", "", "",
    "T_RA1H_Y", "T_RA1H_X", "", "T_RA1H_Z",
    "",
    "RA1H_RA2U", "ra_wr_e_f",
    "ra_cmc1_ad_ab", "ra_cmc1_f_e", "ra_mcp1_f_e",
    "ra_mcp2_e_f", "", "ra_dip2_e_f",
    "ra_mcp3_e_f", "ra_pip3_e_f", "ra_dip3_e_f",
    "ra_mcp4_e_f", "ra_pip4_e_f", "ra_dip4_e_f",
    "ra_mcp5_e_f", "ra_pip5_e_f"};

const std::vector<std::string> RPS_DOF_NAMES = {
    "rps_tra1", "rps_tra2", "rps_tra3",
    "rps_rot1", "rps_rot2", "rps_rot3"
};

const std::vector<mjtNum> RPS_DOF_MULTIPLIERS = {
    1, 1, 1,
    mjPI/180, mjPI/180, mjPI/180
};

const std::vector<std::string> MEASURED_FORCES_SEGMENT_NAMES_FILE = {
    "Thumb", "Index", "Middle", "Ring", "Pinky"
};

const std::vector<std::string> MEASURED_FORCES_SEGMENT_NAMES_MODEL = {
    "RA6D1", "RA6D2", "RA6D3", "RA6D4", "RA6D5"
};
...
std::vector<std::string> load_csv_file(const std::string filename, std::vector<mjtNum>& nTime_ps,
                                       std::vector<std::vector<mjtNum>>& nPos_ps)
{
    std::ifstream infile(filename);
    std::string line, buf;

    // first line
    std::getline(infile, line);  // description
    std::stringstream ls(line);

    std::getline(ls, buf, ',');  // time
    //std::cout<< buf << std::endl;
    std::vector<std::string> segment_names;
    for (std::string e; std::getline(ls, e, ','); ) {
        //std::cout << e << std::endl;
        segment_names.push_back(e);
    }
    ...
    while (std::getline(infile, line)) {
        ls = std::stringstream(line);
        std::getline(ls, buf, ',');
        nTime_ps.push_back(std::atof(buf.c_str()));

        nPos_ps.push_back(std::vector<mjtNum>());
        for (std::string e; std::getline(ls, e, ','); ) {
            nPos_ps.back().push_back(std::atof(e.c_str()));
        }
    }
...
std::vector<std::string> get_loaded_model_dofnames()
{
    char buf[mjMAXUINAME];
    std::vector<std::string> model_dofnames;
    for (int j = 0; j < m->nq; ++j) {
        mju_strncpy(buf, m->names+m->name_jntadr[j], mjMAXUINAME);
        model_dofnames.push_back(buf);
    }
    return model_dofnames;
}

std::vector<int> make_dof_map(const std::vector<std::string> loaded_dof_names)
{
    std::vector<int> dof_map;
    for (int i = 0; i < loaded_dof_names.size(); ++i)
        dof_map.push_back(-1);

    std::vector<std::string> model_dofnames = get_loaded_model_dofnames();

    for (int i = 0; i < loaded_dof_names.size(); ++i) {
        int i_in_osim_list = -1;
        // find this DOF in the opensim list
        for (int j=0; j < DOF_MAP_OPENSIM.size(); ++j) {
            if (DOF_MAP_OPENSIM[j] == loaded_dof_names[i]) {
                i_in_osim_list = j;
                break;
            }
        }
        if (i_in_osim_list >= 0) {
            // if found, find it in the mujoco list
            for (int j = 0; j < model_dofnames.size(); ++j) {
                if (model_dofnames[j] == DOF_MAP_MUJOCO[i_in_osim_list]){
                    dof_map[i] = j;
                    break;
                }
            }
        }
        std::cout << loaded_dof_names[i] << " " << dof_map[i] << std::endl;
    }
...
void set_posture(const int M, const std::vector<int> dof_map, const mjtNum* nPosture,
                 const std::vector<mjtNum> dof_multiplier, const std::vector<mjtNum> dof_offset)
{
    for (int i = 0; i < M; ++i)
        if (dof_map[i] >= 0) {
            ...
            //std::cout << dof_map[i] << " " << d->qpos[dof_map[i]] << std::endl;
        }
}


std::vector<mjtNum> get_position(const int M, const int N,
                                 const std::vector<int> dof_map, const mjtNum* nPos,
                                 const int current_time_point,
                                 const std::vector<mjtNum> dof_multiplier, const std::vector<mjtNum> dof_offset) {
    std::vector<mjtNum> pos;
    for (int i = 0; i < m->nq; ++i)
        pos.push_back(d->qpos[i]);

    int ctp = current_time_point;
    if (ctp < 0)
        ctp = 0;
    if (ctp >= N)
        ctp = N-1;
    ...
    for (int i = 0; i < M; ++i)
        if (dof_map[i] >= 0)
            pos[dof_map[i]] = dof_multiplier[dof_map[i]]*(nPos+M*ctp)[i] + dof_offset[dof_map[i]];

    return pos;
}

std::vector<mjtNum> differential(const std::vector<mjtNum> left, const std::vector<mjtNum> right, mjtNum dt)
{
...
void set_pos_vel_acc(const int M, const int N, const mjtNum dt,
                     const std::vector<int> dof_map, const mjtNum* nPos, const int current_time_point,
                     const std::vector<mjtNum> dof_multiplier, const std::vector<mjtNum> dof_offset) {
    // 3 points of position
    std::vector<mjtNum> pos = get_position(M, N, dof_map, nPos, current_time_point, dof_multiplier, dof_offset);

    std::vector<mjtNum> pos_left = get_position(M, N, dof_map, nPos, current_time_point-1, dof_multiplier, dof_offset);
    std::vector<mjtNum> pos_right = get_position(M, N, dof_map, nPos, current_time_point+1, dof_multiplier, dof_offset);
    ...
    for (int i=0; i<vel_left.size(); i++)
        vel.push_back((vel_left[i]+vel_right[i])/2); // average velocity from left and right estimate
...
void add_acc(const int M, const int N, const mjtNum dt,
                     const std::vector<int> dof_map, const mjtNum* nPos, const int current_time_point,
                     const std::vector<mjtNum> dof_multiplier, const std::vector<mjtNum> dof_offset) {
    // 3 points of position
    std::vector<mjtNum> pos = get_position(M, N, dof_map, nPos, current_time_point, dof_multiplier, dof_offset);
    ...
    for (int i = 0; i < m->nq; ++i) {
        // the acceleration that is providing the movement plus an acceleration that would have counterbalanced the external force
        d->qacc[i] = acc[i] - d->qacc[i];
    }
}


std::vector<int> get_file_rps_dof_order(const std::vector<std::string> loaded_dof_names)
{
    std::vector<int> res;
    for (auto e: RPS_DOF_NAMES) {
        for (int i=0; i<loaded_dof_names.size(); i++) {
            if (e == loaded_dof_names[i]){
                res.push_back(i);
                break;
            }
        }
...
std::vector<mjtNum> get_mean_rps_pos(const int M, const int N, const mjtNum* nPos, const std::vector<std::string> loaded_dof_names)
{
    std::vector<int> file_rps_dof_order = get_file_rps_dof_order(loaded_dof_names);

    std::vector<mjtNum> res;
    mjtNum resbuf;
    for (int i=0; i < file_rps_dof_order.size(); i++){
        resbuf = 0;
        for (int j=0; j<N; j++)
            resbuf += (nPos+j*M)[file_rps_dof_order[i]];
        resbuf /= N;
        resbuf *= RPS_DOF_MULTIPLIERS[i];
        res.push_back(resbuf);
    }
    return res;
}

std::vector<int> get_model_rps_dof_order()
{
    std::vector<std::string> model_dofnames = get_loaded_model_dofnames();
    ...
    for (auto e: RPS_DOF_NAMES) {
        resbuf = -1;
        for (int i=0; i < model_dofnames.size(); i++) {
            if (e == model_dofnames[i]) {
                resbuf = i;
                break;
            }
        }
...
void set_rps_position(std::vector<mjtNum> pos, std::vector<int> model_rps_dof_order)
{
    for (int i=0; i<pos.size(); i++ ) {
        if (model_rps_dof_order[i] >= 0)
            d->qpos[model_rps_dof_order[i]] = pos[i];
    }
}

std::vector<int> make_ps_segment_map(const std::vector<std::string> segment_names)
{
    std::vector<int> ps_segment_map;
    for (int i = 0; i < segment_names.size(); ++i)
        ps_segment_map.push_back(-1);

    for (int i = 0; i < segment_names.size(); ++i) {
        int i_in_model_list = -1;

        // find this DOF in the model list
        for (int j=0; j < MEASURED_FORCES_SEGMENT_NAMES_FILE.size(); ++j) {
            if (MEASURED_FORCES_SEGMENT_NAMES_FILE[j] == segment_names[i]) {
                i_in_model_list = j;
                break;
            }
        }

        if (i_in_model_list >= 0) {
            // if found, find it in the mujoco list
            ps_segment_map[i] = mj_name2id(m, mjOBJ_BODY, MEASURED_FORCES_SEGMENT_NAMES_MODEL[i_in_model_list].c_str());
            std::cout << segment_names[i] << " " << MEASURED_FORCES_SEGMENT_NAMES_MODEL[i_in_model_list] << " " << ps_segment_map[i] << std::endl;
        }
    }

    return ps_segment_map;
}


std::vector<mjtNum> find_closest_force(const mjtNum currentTime, const std::vector<mjtNum>& nTime_ps,
                                       const std::vector<std::vector<mjtNum>>& nPos_ps)
{
    int i;
    for (i=0; i<nTime_ps.size()-1; i++) {
        if (nTime_ps[i] - nTime_ps[0] >= currentTime)
            break;
    }
    //std::cout << nTime_ps[i] - nTime_ps[0] << " ";
    return nPos_ps[i];
}

void apply_segment_force(const std::vector<mjtNum>& current_segment_force,
                         const std::vector<int>& ps_segment_map,
                         const std::vector<std::vector<mjtNum>>& force_vectors)
{
    int csi;
    for (int i=0; i < current_segment_force.size(); i++) {
        csi = ps_segment_map[i];
        d->xfrc_applied[6*csi] = current_segment_force[i] * force_vectors[i][0];
        d->xfrc_applied[6*csi+1] = current_segment_force[i] * force_vectors[i][1];
        d->xfrc_applied[6*csi+2] = current_segment_force[i] * force_vectors[i][2];
    }
}
...
int main(int argc, const char** argv)
{
    ...
    printf("This program loads kinematics and kinetics from approximate adjusted inverse kinematics, applies the forces to fingers and outputs inverse torques.\n");

    //--------------- always load model
    if (argc < 2) {
            sprintf(filename, "%s/stereo_inverse_kinematics/mjc_model_daiquiri/aah_daiquiri_att_rps.xml",
    ...
    std::vector<std::string> dof_names = load_mot_file(
        "C:/Repositories/stereo_inverse_kinematics/2020_10_grant_proc/Daiquiri data/2020-10-28/joint_angles/trial50_processed_bounded.mot", &M, &N, &nTime, &nPos);
    std::vector<int> dof_map = make_dof_map(dof_names);
    ...
    dof_multiplier[1] *= 0.2;
    dof_offset[3] = -mjPI*0.5;
    set_posture(M, dof_map, nPos, dof_multiplier, dof_offset); // put into the first posture

    // RPS:
    std::vector<mjtNum> mean_rps_pos = get_mean_rps_pos(M, N, nPos, dof_names);
    std::vector<int> file_rps_dof_order = get_file_rps_dof_order(dof_names);
    std::vector<int> model_rps_dof_order = get_model_rps_dof_order();
    set_rps_position(mean_rps_pos, model_rps_dof_order);

    std::cout << "Average file_rps_dof_order of RPS: ";
    for (auto e: file_rps_dof_order)
        std::cout << e << " ";
    std::cout << std::endl;

    std::cout << "Average postion of RPS: ";
    for (auto e: mean_rps_pos)
        std::cout << e << " ";
    std::cout << std::endl;

    // load kinetics
    std::vector<mjtNum> nTime_ps;
    std::vector<std::vector<mjtNum>> nPos_ps;
    // open file with Liza-maually calculated forces applied forces
    std::vector<std::string> segment_names = load_csv_file(
        "C:/Repositories/stereo_inverse_kinematics/2020_10_grant_proc/Daiquiri data/2020-10-28/pressure_sensor/trial50_forces_for_anton.csv", nTime_ps, nPos_ps);
    std::cout << "Loaded " << nTime_ps.size() << " time points of force measurements from " << segment_names.size() << " segments: ";
    for (auto e: segment_names)
        std::cout << e << ", ";
    std::cout << std::endl;
    // assign them specific objects to act upon - distal finger segments
    std::vector<int> ps_segment_map = make_ps_segment_map(segment_names);
    // figure out the vector of the force
    std::vector<mjtNum> force_vector = {0, 0, 1};
    std::vector<std::vector<mjtNum>> force_vectors;
    for (auto e : ps_segment_map)
        force_vectors.push_back(force_vector);
    // thumb has a twisted orientation
    force_vectors[0] = std::vector<mjtNum>{-0.7, 0, 0.7};
    std::vector<mjtNum> current_segment_force;

    mj_forward(m, d);

    int current_time_point = 0;
    mjtNum dt = nTime[1] - nTime[0];
    int sleep_time = dt*1000;

    std::cout << "Time-step " << sleep_time << "ms" << std::endl;

settings.run = 1;
    bool ran_once = false;
    std::vector<mjtNum> inverse_torques;

    // open output file
    std::ofstream fo("C:/Repositories/stereo_inverse_kinematics/2020_10_grant_proc/Daiquiri data/2020-10-28/joint_angles/trial50_inverse_torques.csv");
    std::vector<std::string> model_dofnames = get_loaded_model_dofnames();
    if (fo.is_open()) {
        fo << "time";
        for (std::string e : model_dofnames)
            fo << "," << e;
        fo << std::endl;
    }
    ...
    while( !glfwWindowShouldClose(window) && !settings.exitrequest )
    {
        ...
        std::cout << "current time point " << current_time_point << "\r";
        // set visualisation posture
        if (settings.run)
            set_posture(M, dof_map, nPos + current_time_point*M, dof_multiplier, dof_offset);
        ...
        // set qpos qvel and qacc
        if (settings.run)
            set_pos_vel_acc(M, N, dt, dof_map, nPos, current_time_point, dof_multiplier, dof_offset);

        // find the forces corresponding to the closest time-point
        current_segment_force = find_closest_force(nTime[current_time_point], nTime_ps, nPos_ps);

        // apply the forces
        apply_segment_force(current_segment_force, ps_segment_map, force_vectors);

        // transforms the forces into acceleration
        mj_forward(m, d);

        // append the kinematic acceleration
        if (settings.run)
            add_acc(M, N, dt, dof_map, nPos, current_time_point, dof_multiplier, dof_offset);

        // calculate inverse torques
        // transforms acceleration into forces
        mj_inverse(m, d);

        inverse_torques.clear();
        for (int i = 0; i < m->nq; ++i)
            inverse_torques.push_back(d->qfrc_inverse[i]);

        if (!ran_once && settings.run) {
            // print torques and forces
            std::cout << "current time point " << current_time_point << " " << nTime[current_time_point] << " " << current_segment_force.size() << "\t";
            // for (auto e: inverse_torques)
            //     std::cout << e << " ";
            for (auto e: current_segment_force)
                std::cout << e << " ";

            std::cout << std::endl;

            // export torques to file
            if (fo.is_open()) {
                fo << nTime[current_time_point];
                for (auto e: inverse_torques)
                    fo << "," << e;
                fo << std::endl;
            }
        ...
        // if (settings.run)  // more responsive when not playing
        //     std::this_thread::sleep_for(std::chrono::milliseconds(sleep_time));

        if (settings.run)
            current_time_point++;
        if (current_time_point == N) {
