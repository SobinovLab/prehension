...
std::vector<mjtNum> get_touch_sensor_vector(const int M)
{
    ...
    for (int i = 2*M; i < m->nsensor; ++i)
        nCurrentSensor.push_back(d->sensordata[i]);
...
mjtNum total_touch_fource(const int M)
{
    mjtNum answ = 0;
    for (int i = 2*M; i < m->nsensor; ++i)
        answ += d->sensordata[i];
...
int main(int argc, const char** argv)
{
    ...
    // always load model
    sprintf(filename,
            "%s/stereo_inverse_kinematics/mjc_model/Hand_18_DOF_meshes_cylinder.xml",
    ...
    for (int i = 0; i < M; ++i)
        nPos0[i] = 0.;
    nPos0[1] = -0.55;  // ra_wr_e_f
    nPos0[2] = 0.87;  // ra_cmc1_f_e
    nPos0[3] = 1.55; // ra_cmc1_ad_ab
    nPos0[4] = 0.87; // ra_cmc1_rot
    nPos0[7] = 0.1;  // ra_mcp2_e_f
    // nPos0[10] = 0.1;  // ra_mcp3_e_f
    // nPos0[13] = 0.1;  // ra_mcp4_e_f
    // nPos0[16] = 0.1;  // ra_mcp5_e_f
    set_posture(M, nPos0);
    printf("Generated and set starting posture with %d DOFs\n", M);

    // set applied forces
    mjtNum* nQfrcApplied = new mjtNum[M];
    mjtNum* nqa = new mjtNum[M];
    for (int i = 0; i < M; ++i)
        nQfrcApplied[i] = 0.;
    mjtNum proximalForce = 0.1*5;
    mjtNum proximalForceBalance = 0.2;
    mjtNum middleForce = 0.004*5;
    mjtNum distalForce = 0.001*5;
    // thumb
    nQfrcApplied[2] = proximalForceBalance*2;  // ra_cmc1_f_e
    nQfrcApplied[3] = -proximalForce; // ra_cmc1_ad_ab
    nQfrcApplied[4] = proximalForceBalance; // ra_cmc1_rot
    nQfrcApplied[5] = -middleForce*1.5; // ra_mcp1_f_e
    nQfrcApplied[6] = -distalForce*1.5; // ra_ip1_f_e
    // mcp
    nQfrcApplied[7] = proximalForce*0.6;  // ra_mcp2_e_f
    nQfrcApplied[10] = proximalForce*0.63;  // ra_mcp3_e_f
    nQfrcApplied[13] = proximalForce*0.6;  // ra_mcp4_e_f
    nQfrcApplied[16] = proximalForce*0.3;  // ra_mcp5_e_f
    // pip
    nQfrcApplied[8] = middleForce*0.6;  // ra_pip2_e_f
    nQfrcApplied[11] = middleForce*0.63;  // ra_pip3_e_f
    nQfrcApplied[14] = middleForce*0.6;  // ra_pip4_e_f
    nQfrcApplied[17] = middleForce;  // ra_pip5_e_f
    // dip
    nQfrcApplied[9] = distalForce*0.6;  // ra_dip2_e_f
    nQfrcApplied[12] = distalForce*0.63;  // ra_dip3_e_f
    nQfrcApplied[15] = distalForce*0.6;  // ra_dip4_e_f
    nQfrcApplied[18] = distalForce*0.3;  // ra_dip5_e_f
    //set_applied_torque(M, nQfrcApplied);
    mjtNum forceMultiplier = 0.;
    mjtNum forceTimeFactor = 2.;  // sec
    ...
    // get sensor names
    std::vector<std::string> touchSensorNames = getTouchSensorNames(M);
    ...
    while( !glfwWindowShouldClose(window) && !settings.exitrequest )
    {
        ...
        printf("Time: %f sec; time to force: %f; total touch force: %f N\r",
               timeFromStart, timeFromStart-forceTimeDelay, total_touch_fource(M));

        // extract sensor data
        if (settings.run) {
            times.push_back(timeFromStart);
            nStoredSensor.push_back(get_touch_sensor_vector(M));
        }
        ...
        if (timeFromStart <= forceTimeDelay)
            forceMultiplier = 0.;
        // else if (timeFromStart <= forceTimeDelay+forceTimeFactor)
        //     forceMultiplier = (timeFromStart-forceTimeDelay) / forceTimeFactor;
        // else if (timeFromStart <= forceTimeDelay+forceTimeFactor*2)
        //     forceMultiplier = (forceTimeDelay+forceTimeFactor*2-timeFromStart) / forceTimeFactor;
        else if (timeFromStart <= forceTimeDelay+forceTimeFactor*2)
            forceMultiplier = sin((timeFromStart-forceTimeDelay) / forceTimeFactor / 2 * PI);
        else
            forceMultiplier = 0;
        for (int i = 0; i < M; ++i)
        ...
        // maintain
        nqa[2] = proximalForceBalance;  // ra_cmc1_f_e
        nqa[4] = proximalForceBalance; // ra_cmc1_rot
        set_applied_torque(M, nqa);
    ...
    // save data
    sprintf(filename, "%s/touch_pad_deeplabcut_rig/inverse_kinematics/mjc_code/res/touchsensors.csv",
            getenv("BL_REPOSITORIES"));
