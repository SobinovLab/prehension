...
//-------------------------------- MY FUNCTIONS ----------------------------------------
void set_posture(const int M, const mjtNum* nPos)
{
    for (int i = 0; i < M; ++i)
        d->qpos[i] = nPos[i];
}

void set_applied_torque(const int M, const mjtNum* nQfrcApplied)
{
    for (int i = 0; i < M; ++i)
        d->qfrc_applied[i] = nQfrcApplied[i];
}

std::vector<mjtNum> get_touch_sensor_vector(const int M)
{
    std::vector<mjtNum> nCurrentSensor;
    mjtNum val;

    for (int i = 2*M; i < m->nsensor; ++i) {
        val = d->sensordata[i];
        // if (val > 1e2)  // instability, usually in the beginning
        //     val = 0;
        nCurrentSensor.push_back(val);
    }

    return nCurrentSensor;
}

std::vector<std::string> getTouchSensorNames(const int M)
{
    char buf[mjMAXUINAME];
    std::vector<std::string> touchSensorNames;
    for (int i = 2*M; i < m->nsensor; ++i)
    {
        mju_strncpy(buf, m->names+m->name_sensoradr[i], mjMAXUINAME);
        touchSensorNames.push_back(std::string(buf));
    }
    return touchSensorNames;
}

void dump_sensor_data(const char* filename,
                      const std::vector<std::string> touchSensorNames,
                      const std::vector<mjtNum> times,
                      const std::vector<std::vector<mjtNum>> nStoredSensor)
{
    std::ofstream fo(filename);

    // header
    fo << "Time";
    for (int i = 0; i < touchSensorNames.size(); ++i) {
        fo << "," << touchSensorNames[i];
    }
    fo << "\n";
    ...
    {
        fo << times[i];
        for (int j = 0; j < nStoredSensor[i].size(); ++j)
            fo << ", " << nStoredSensor[i][j];
        fo << "\n";
    }
...
mjtNum total_touch_force(const int M)
{
    mjtNum answ = 0;
    for (int i = 2*M; i < m->nsensor; ++i)
        answ += d->sensordata[i];
...
mjtNum sum_vector(const std::vector<mjtNum> v){
    mjtNum answ = 0.;
    for (mjtNum val: v)
        answ += val;
    return answ;
...
mjtNum* loadDefaultPosture(const char* filename, const int M)
{
    std::ifstream f;
    f.open (filename);

    if (!f.is_open()) {
        printf("Could not open default posture file %s.\n", filename);
        return NULL;
    }

    std::string dof_names_line, dof_vals_line, dof_name, val_s;
    char buf[mjMAXUINAME];
    std::getline(f, dof_names_line);
    std::getline(f, dof_vals_line);

    mjtNum* nPos0 = new mjtNum[M];
    for (int i = 0; i < M; ++i)
        nPos0[i] = 0.;

    bool found_joint;
    std::stringstream dof_names_ss(dof_names_line);
    std::stringstream dof_vals_ss(dof_vals_line);
    while (getline(dof_names_ss, dof_name, ',') && getline(dof_vals_ss, val_s, ',')) {
        found_joint = false;
        for (int i = 0; i < m->njnt; ++i)
        {
            mju_strncpy(buf, m->names+m->name_jntadr[i], mjMAXUINAME);
            //printf("\t%s\n", buf);
            if (!dof_name.compare(buf)) {
                nPos0[m->jnt_qposadr[i]] = std::atof(val_s.c_str());
                found_joint = true;
                break;
            }
        }
        if (!found_joint)
            printf("Could not find joint %s in the loaded model.\n", dof_name.c_str());
    }
    return nPos0;
}

mjtNum* loadDefaultTorque(const char* filename, const int M)
{
    std::ifstream f;
    f.open (filename);

    if (!f.is_open()) {
        printf("Could not open default torque file %s.\n", filename);
        return NULL;
    }
    ...
    mjtNum* nQfrcApplied = new mjtNum[M];
    for (int i = 0; i < M; ++i)
        nQfrcApplied[i] = 0.;
    ...
    while (getline(dof_names_ss, dof_name, ',') && getline(dof_vals_ss, val_s, ',')) {
        ...
        for (int i = 0; i < m->njnt; ++i)
        {
            mju_strncpy(buf, m->names+m->name_jntadr[i], mjMAXUINAME);
            if (!dof_name.compare(buf)) {
                nQfrcApplied[m->jnt_qposadr[i]] = std::atof(val_s.c_str());
                found_joint = true;
                break;
            }
        }
...
int main(int argc, const char** argv)
{
    ...
    // always load model
    sprintf(filename,
            "%s/stereo_inverse_kinematics/mjc_models/mjc_model/Hand_18_DOF_meshes_cylinder_compliance.xml",
    ...
    int M = m->nq;
    // hardcoded number of joints actually in the model of the hand and not a composite object
    int Mmodel = 19;
    mjtNum* nPos0 = loadDefaultPosture("../controls/default_pos.csv", M);
    set_posture(Mmodel, nPos0);
    printf("Generated and set starting posture with %d DOFs\n", Mmodel);

    // Default applied torque profile.
    // TODO fix
    mjtNum* nQfrcApplied = loadDefaultPosture("../controls/default_grasp_torque.csv", M);
    mjtNum* nqa = new mjtNum[Mmodel];

    //set_applied_torque(Mmodel, nQfrcApplied);
    mjtNum forceMultiplier = 0.;
    mjtNum forceTimeFactor = 1.5;  // sec
    mjtNum forceTimeDelay = 0.;  // sec
    mjtNum totalForceThreshold = 100; // N
    mjtNum totalForceTarget = 2.5; // N
    mjtNum holdPeriod = 0.5; // sec
    mjtNum endTime = forceTimeDelay + forceTimeFactor + holdPeriod + 2 * forceTimeFactor;
    printf("Generated force params\n");

    std::vector<std::vector<mjtNum>> nStoredSensor;
    std::vector<mjtNum> times;
    std::vector<mjtNum> touch_sensor_vector;
    mjtNum totalForce;
    mjtNum maxReachedForceMultiplier;

    // get sensor names
    std::vector<std::string> touchSensorNames = getTouchSensorNames(Mmodel);
    printf("Found sensors: ");
    for (int i = 0; i < touchSensorNames.size(); ++i)
        printf("%s, ", touchSensorNames[i].c_str());
    printf("\n");

    // start simulation thread
    settings.run = 1;
    std::thread simthread(simulate);
    printf("Started simulation thread\n");

    // get time
    mtx.lock();
    mjtNum timeStart = d->time;
    mtx.unlock();
    mjtNum timeFromStart = -1000;
    mjtNum targetAchievedTime = -1;
    int stage = 0;
    bool newTimepoint = false;
    const bool constantTorque = true;
    const bool pidController = true;
    mjtNum targetRelForceNormed;
    const double controlPow = 2;
    const mjtNum pidForceMultiplier = 5;
    const mjtNum torqueBeforeContact = pow(0.05 / pidForceMultiplier, 1./controlPow) ;
    ...
    while( !glfwWindowShouldClose(window) && !settings.exitrequest )
    {
        ...
        // print runtime
        if (d->time - timeStart < timeFromStart) {
            printf("\nSomething broke, aborting simulation.\n");
            settings.exitrequest = 1;
            newTimepoint = false;
        }
        else if (d->time - timeStart == timeFromStart)
            newTimepoint = false;
        else
            newTimepoint = true;
        timeFromStart = d->time - timeStart;

        // get sensor data
        if (settings.run && newTimepoint) {
            touch_sensor_vector = get_touch_sensor_vector(Mmodel);
            if (timeFromStart <= forceTimeDelay)
                for (int i_sensor = 0; i_sensor < touch_sensor_vector.size(); ++i_sensor)
                    touch_sensor_vector[i_sensor] = 0.;
            for (int i_sensor = 0; i_sensor < touch_sensor_vector.size(); ++i_sensor)
                if (touch_sensor_vector[i_sensor] > 100)  // instability probably
                    touch_sensor_vector[i_sensor] = 0.;

            totalForce = sum_vector(touch_sensor_vector);
            totalForce = touch_sensor_vector[4];
            printf("Time: %f sec; time to force: %f; total touch force: %f N\t\t\r",
                   timeFromStart, timeFromStart-forceTimeDelay, totalForce);

        // extract sensor data
            times.push_back(timeFromStart);
            nStoredSensor.push_back(touch_sensor_vector);
        }
        ...
        // calculate torque
        // from forceTimeDelay until forceTimeDelay+forceTimeFactor we are linear rising force
        if (timeFromStart <= forceTimeDelay) {
            forceMultiplier = 0.;
            targetRelForceNormed = 0.;
        }
        else if (timeFromStart <= forceTimeDelay+forceTimeFactor && totalForce < totalForceThreshold){
            if (constantTorque)
                forceMultiplier = 0.2;
            else
                forceMultiplier = (timeFromStart-forceTimeDelay) / forceTimeFactor;

            targetRelForceNormed = (totalForceTarget - totalForce) / totalForceTarget;
            if (stage == 0) {
                printf("\nStarted applying torque.\n");
                stage++;
            }
            if (totalForce == 0)  // softer before contact
                targetRelForceNormed = torqueBeforeContact;
        }

        // if the threshold has been crossed or time elapsed, start the counter
        if (targetAchievedTime == -1 && (totalForce >= totalForceThreshold || timeFromStart > forceTimeDelay + forceTimeFactor)) {
            targetAchievedTime = timeFromStart;
            maxReachedForceMultiplier = forceMultiplier;
            printf("\nTarget force reached.\n");
            stage++;
        }

        if (targetAchievedTime != -1 && timeFromStart > targetAchievedTime && timeFromStart <= targetAchievedTime + holdPeriod) {
            targetRelForceNormed = (totalForceTarget - totalForce) / totalForceTarget;
        }

        // if the counter has started and hold time has passed, start lowering the torque
        if (targetAchievedTime != -1 && timeFromStart >= targetAchievedTime + holdPeriod) {
            if (constantTorque)
                forceMultiplier = -0.2;
            else {
                forceMultiplier = (forceTimeFactor - (timeFromStart - targetAchievedTime - holdPeriod)) / forceTimeFactor * maxReachedForceMultiplier;
                forceMultiplier = mjMAX(forceMultiplier, -1);
            }
            if (stage == 2) {
                printf("\nHold period passed, lowering torque.\n");
                stage++;
            }
            targetRelForceNormed = (0 - totalForce) / totalForceTarget;
        }

        if (pidController) {
            targetRelForceNormed = mjMIN(targetRelForceNormed, 1);
            targetRelForceNormed = mjMAX(targetRelForceNormed, -1);
            if (controlPow != 1)
                if (targetRelForceNormed < 0)
                    targetRelForceNormed = -pow(-targetRelForceNormed, controlPow);
                else
                    targetRelForceNormed = pow(targetRelForceNormed, controlPow);
            forceMultiplier = targetRelForceNormed * pidForceMultiplier;
        }

        // apply torques
        for (int i = 0; i < Mmodel; ++i)
            nqa[i] = forceMultiplier * nQfrcApplied[i];
        set_applied_torque(Mmodel, nqa);
    ...
    // save data
    sprintf(filename, "%s/stereo_inverse_kinematics/mjc_code/res/touchsensors_compliance.csv",
            getenv("BL_REPOSITORIES"));
    printf("Dumping data into %s.\n", filename);
    dump_sensor_data(filename, touchSensorNames, times, nStoredSensor);
