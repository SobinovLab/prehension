...
class VideoViewInterface:
    ...
    def display_videos_frame(self, i_frame=None):
        ...
        for camera, ax in self.video_axes.items():
            ...
            self.videosframe[camera] = self.display_videoframe(self.videos[camera], ax, i_frame)
    ...
    def on_release(self, event):
        if (
            self.selecting_camera is not None
            and event.inaxes == self.video_axes[self.selecting_camera]
        ):
            self.clear_zoom_camera_markers()
            ...
            if self.selected_marker is not None and self.selecting_zoom_left:
                ...
                for marker in self.markers:
                    ...
                    if (
                        p is not None
                        and np.sqrt((event.xdata - p[0]) ** 2 + (event.ydata - p[1]) ** 2)
                        < DELETE_MARKER_RADIUS
                    ):
