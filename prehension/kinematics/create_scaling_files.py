...
def prepare_scaling_files(trial, session, mstruct, period):
    ...
    bs_period = [best_score_time - out_halfwindow,
                 best_score_time + out_halfwindow]
    if period is None or len(period) == 0:
        period = bs_period
        rs('Using best estimated period: [{}, {}]'.format(
            period[0], period[1]))
    else:
        rs('Using provided period: [{}, {}]'.format(
            period[0], period[1]))
    ...
    tools.actual_vline(
        ax4, bs_period[0], color=micolors['purple'][600], linestyle='--')
    tools.actual_vline(
        ax4, bs_period[1], color=micolors['purple'][600], linestyle='--')
    ...
    rs('Found start frame {} and end frame {}.'.format(
    ...
    # create SC file
    tool_name = session + '_' + \
        os.path.basename(trial.scaling_sc_filename)[
            :-4] + '_scaling_tool'
...
def transfer_position_to_model(trial, mstruct, mdof):
    ...
    dof_names, _, dofs = io_tools.import_mot(
        trial.scaling_kinematic_filename)
...
def create_scaling_files(server, session, trial_number, temp, overwrite, period, transfer_position):
    ...
    if transfer_position:
        ...
        if trial.do_scaling_files_exist() and not overwrite:
            ws('Scaling files for trial {} already exist, aborting.'.format(
                trial_number))
            return
