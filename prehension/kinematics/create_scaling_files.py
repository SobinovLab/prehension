...
def fill_nans(v):
    ...
    for iv, va in enumerate(v):
        if np.isnan(va):
            v[iv] = v[prev_nonnan_idxs[iv]] + (
                v[next_nonnan_idxs[iv]] - v[prev_nonnan_idxs[iv]]
...
def prepare_scaling_files(trial, session, mstruct, period):
    # load trc
    bodyparts, _, times, points, rate, units = io_tools.import_trc(trial.markers_3D_filename_trc)
    ...
    rs(
        'Total {} non-pressure sensors left:'.format(
    ...
    for smg, markers in SEGMENT_MARKER_GROUPS.items():
        rs('\t{}: {}'.format(smg, ', '.join(m for m in markers if m in bodyparts)))
    # save trc
    io_tools.export_trc(
    ...
    time_range = [0, 1.0 / rate * len(points)]
    ...
    )  # will run on the open model
    inverse_kinematics.make_ik_file(
        trial.scaling_ik_filename,
        ik_xml_str,
        marker_weights,
...
def transfer_position_to_model(trial, mstruct, mdof):
    ...
    rs(
        'Transferred median position from {} into model {}.'.format(
...
def create_scaling_files(server, session, trial_number, temp, overwrite, period, transfer_position):
    ...
    if not trial.do_3d_files_exist():
        ValueError(
            'Trial {} does not have 3D files to use for IK and scaling.'.format(trial_number)
        )
    ...
    if transfer_position:
        ...
        if not os.path.exists(trial.scaling_kinematic_filename):
            ValueError(
                'Scaling joint angle file for trial {} does not exists.'.format(trial_number)
            )
