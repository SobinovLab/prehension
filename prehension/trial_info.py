...
# -*- coding: utf-8 -*-
"""
A class defining all information used for a trial, like the file locations.
...
class TrialInfo:
    ...
    def generate_filenames(self, mstruct):
        ...
        self.markers_2D_dirname = os.path.join(mstruct["markers_2D_dir"], trial_name)
        self.dlc_filemasks = {
            k: os.path.join(self.markers_2D_dirname, v + "*") for k, v in mstruct["cameras"].items()
        }
        ...
        self.scaling_ik_filename = os.path.join(mstruct["scaling_dir"], trial_name + "_IK.xml")
        self.scaling_sc_filename = os.path.join(mstruct["scaling_dir"], trial_name + "_SC.xml")
        self.scaling_kinematic_filename = os.path.join(mstruct["scaling_dir"], trial_name + ".mot")
        ...
        self.markers_3D_filename_csv = os.path.join(mstruct["markers_3D_dir"], trial_name + ".csv")
        self.markers_3D_filename_trc = os.path.join(mstruct["markers_3D_dir"], trial_name + ".trc")
        self.ik_filename = os.path.join(mstruct["pre_ja_dir"], trial_name + "_IK.xml")
        self.base_ik_filename = os.path.join(mstruct["pre_ja_dir"], trial_name + "_base_IK.xml")
        self.ik_log_filename = os.path.join(mstruct["pre_ja_dir"], trial_name + ".log")

        # before filtering and aligning
        self.pre_kinematic_filename = os.path.join(mstruct["pre_ja_dir"], trial_name + ".mot")
        self.base_kinematic_filename = os.path.join(mstruct["pre_ja_dir"], trial_name + "_base.mot")
        # needs .mot or .csv
        self.post_kinematic_filename_mot = os.path.join(mstruct["post_ja_dir"], trial_name + ".mot")
        self.post_kinematic_filename_csv = os.path.join(mstruct["post_ja_dir"], trial_name + ".csv")

        # created by MuJoCo adjustment program
        self.adjustment_kinematic_filename = os.path.join(
            mstruct["post_ja_dir"], trial_name + mstruct["kin_adjustment_suffix"] + ".csv"
        )
        ...
        self.digit_forces_filename = os.path.join(mstruct["digit_forces_dir"], trial_name + ".csv")
        self.segment_forces_filename = os.path.join(
            mstruct["segment_forces_dir"], trial_name + ".csv"
        )

        self.mujoco_video = os.path.join(mstruct["mujoco_videos_dir"], trial_name + ".mp4")
    ...
    def get_dlc_filenames_all(self):
        return {k: TrialInfo.get_dlc_filenames(v) for k, v in self.dlc_filemasks.items()}
    ...
    def get_2d_filenames(self):
        return {k: TrialInfo.get_2d_filename(v) for k, v in self.markers_2D_filemasks.items()}
