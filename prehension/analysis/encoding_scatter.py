...
def _load_available(processed_server, session, predictors, use_threshold_crossings, min_adj_r2,
    ...
    `min_adj_r2` (poorly-encoded units are dropped).  Filtered to `predictors` when given, and
    to each predictor's effective period (resolve_period(predictor, period); files written
    before the period feature have no 'period' key and count as PERIOD_ALL).
    """
    out = {}
...
def _pooled_pairs(processed_server, sessions, predictors, use_threshold_crossings, min_adj_r2,
    ...
    predictors have saved encoding for the requested period.
    """
    ...
    if len(preds) < 2:
        ...
        raise ValueError(
            ...
            '{}. Generate the encoding for these periods (encoding.py --period ...) or pass a '
            'matching --period.'.format(preds, want))
...
def encoding_scatter(processed_server, sessions, predictors=None, min_adj_r2=MIN_ADJ_R2,
    ...
    on different periods -- pass a single period for a like-for-like comparison).  Saved by
    default into <processed_server>/pooled_figures/encoding_scatter.  Returns the figure (or
