...
def load_maps(trial):
    if os.path.exists(trial.lps_map_filename):
        lps_digit_mask = np.array(
            import_one_csv_matrix(trial.lps_map_filename, dtype=int))
    else:
        raise ValueError('Map does not exist.')
    if os.path.exists(trial.rps_map_filename):
        rps_digit_mask = np.array(
            import_one_csv_matrix(trial.rps_map_filename, dtype=int))
    else:
        raise ValueError('Map does not exist.')
...
def load_forces(mstruct, trial):
    ...
    for ps_name in mstruct['ps_dic'].keys():
        ps_times, ps_matrices[ps_name] = import_matrices(
            trial.get_post_ps_filenames()[ps_name])
        matched_contacts[ps_name] = import_matched_contacts(
...
def get_summed_force_data(tsm1_file, tsm2_file, verbose=False):
    ...
    ps_times1, ps_matrices1 = import_tsm_matrix(tsm1_file)  # import_matrices(tsm1_file)
    ps_times2, ps_matrices2 = import_tsm_matrix(tsm2_file)  # import_matrices(tsm2_file)
