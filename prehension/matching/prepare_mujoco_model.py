...
def tessellate_sensor(wb, name, hand_geomnames, sense_distance,
                      n=(44, 44), sense_box_rgba=(0.2, 0.2, 0.2, 1),
                      mask=None, leave_edges=False, inverse_x=False, inverse_y=False,
                      switch_xy=False):
    ...
    for ix, iy in itertools.product(range(n[0]), range(n[1])):
        if mask is None or mask[ix][iy] or (leave_edges and (ix in (0, n[0]) or iy in (0, n[1]))):
            ...
            sense_box = make_segment(
                sense_box_name, [x, y, 0], box_int_size,
                sense_box_rgba, 3, geomname=sense_box_geomname)
            att.findall('*')[-1].tail = '\n' + ' '*(2*4)
            att.append(sense_box)
            contacts += add_pair_contacts(sense_box_geomname, hand_geomnames,
                                          margin=sense_distance, gap=sense_distance)
            n_sensels += 1
...
def tessellate_sensors(mjc_model, out_model, sense_distance,
                       nconmax=100000, njmax=10000, nstack=3000000,
                       left_ps='LPS', right_ps='RPS', hand_parent_bname='RA3C',
    ...
    root = tree.getroot()
    if root.tag != 'mujoco':
        raise ValueError('Wrong structure of the MuJoCo model file.'
                         ' Tag mujoco is not present at top level.')
    ...
    contacts = tessellate_sensor(wb, left_ps, hand_geomnames, sense_distance,
                                 ...
                                 switch_xy=True, inverse_x=True)
    contacts += tessellate_sensor(wb, right_ps, hand_geomnames, sense_distance,
                                  sense_box_rgba=right_color, mask=right_ps_mask,
                                  switch_xy=True, inverse_x=True)
...
def prepare_mujoco_model(server, sessions, trials_sel, temp, overwrite,
    ...
    for session in tqdm.tqdm(sessions, ncols=100, desc='Sessions'):
        ...
        if tessellate:
            ...
            tessellate_sensors(mstruct['mujoco_model'], mstruct['mujoco_model_sensorized'],
                               sense_distance,
