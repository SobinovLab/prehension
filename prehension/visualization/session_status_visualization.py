...
def display_session_info(experimental_server_wrappers, training_server_wrappers, clean, last_n):
    ...
    def check_proc_folders(proc_path):
        ...
        prehension_plots_exist_bool = [os.path.exists(os.path.join(proc_path,
                                                      "prehension_plots", fname))
        ...
        if all(prehension_plots_exist_bool):
            ...
            prehension_symbol = (f" {Fore.YELLOW}{sum(prehension_plots_exist_bool)}/"
                                 f"{len(EXPECTED_PLOT_NAMES)}{Style.RESET_ALL} ")
        else:
    ...
    def can_delete(sw, delete=False):
        ...
        matching_logs_other = set(glob.glob(os.path.join(os.path.dirname(os.path.dirname(
            sw.raw_ss)), "**", logname), recursive=True)) - set([logname_full,])
