...
# -*- coding: utf-8 -*-
"""
Provides utilities for session data visualization, such as heatmaps and forcetraces
...
def create_heatmap_from_trials_list(tr_list, cmap, max_discrete_conds=9, bin_width_N=1,
    ...
    if use_force_bins:
        ...
        force_bins = []
        for lo_bound in range(rounded_min_cond, rounded_max_cond, bin_width_N):
    ...
    def plot_force_traces(
        ...
        for ref_event in ref_events:
            ...
            for trial in trials_flattened:
                ...
                ), f"shifted times check 1 is type ({type(trial.tsm_times)})"
                valid_idx = (shifted_times >= -pre_event_pad) & (shifted_times <= post_event_pad)
                assert isinstance(
                    shifted_times, np.ndarray
                ), f"shifted times check 2 is type ({type(trial.tsm_times)})"
                ...
                # 3. Interpolate
                fxn = interp1d(
                    shifted_times, interp_forces, kind="linear", fill_value="extrapolate"
                )
            ...
            _, maxy = ax.get_ylim()
            ax.text(0, 0.6 * maxy, ref_event, rotation=90, va="bottom", ha="right")
