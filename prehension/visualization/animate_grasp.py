...
def get_matched_contacts(mstruct, trial):
    ...
    for ps_name in mstruct['ps_dic'].keys():
        ...
        residual_force[ps_name] = np.sum(
            np.logical_not(matched_sensels) * np.array(ps_matrix), axis=(1, 2)
        )
...
class GraspAnimation:
    def __init__(
        self, trial, ps_times, ps_matrices, lps_video, rps_video, fps, num_frames, data_digits_aps
    ):
        self.trial = trial
        ...
        self.matched_contacts = {
            LPS_NAME: io.import_matched_contacts(self.trial.matched_contacts_filenames[LPS_NAME]),
    ...
    def generate_internal_data(self):
        ...
        self.sdg = {
            k: v
            for ik, (k, v) in enumerate(SEGMENT_DIGIT_GROUPS.items())
...
def animate_grasp_all(mstruct, trial):
    ...
    ga = GraspAnimation(
        trial, ps_times, ps_matrices, video, video_rps, fps, num_frames, data_digits_aps
    )
