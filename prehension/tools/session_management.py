...
def fetch_server_session_dirs(preset, sessions=[], filter=False):
    ...
    if not sessions:
        exp_session_names = meta_session.find_session_dirs(
            preset['default_server'])
        train_session_names = meta_session.find_session_dirs(
    ...
    if filter:
        experimental_raw_proc_pairs = _filter_pairs(
            raw_server_sessions, proc_server_sessions)
    else:
        experimental_raw_proc_pairs = list(
            zip(raw_server_sessions, proc_server_sessions))
    ...
    if filter:
        training_raw_proc_pairs = _filter_pairs(
            raw_training_sessions, proc_training_sessions)
    else:
        training_raw_proc_pairs = list(
            zip(raw_training_sessions, proc_training_sessions))
...
def does_trianing_servers_exist(preset, verbose=False):
    ...
    assert all(
        k in preset for k in keys), 'Missing keys in preset: {}'.format(keys)

    training_servers_not_none = all(
        v is not None for v in (preset[k] for k in keys))

    training_servers_exist = False
    if training_servers_not_none:
        training_servers_exist = all(os.path.exists(
            v) for v in (preset[k] for k in keys))

    if not training_servers_not_none and verbose:
        ws('one or more training servers are None in preset: {}'.format(
            preset['names'][0]))

    if not training_servers_exist and verbose:
        ws('one or more training servers do not exist in preset: {}'.format(
            preset['names'][0]))
...
def apply_to_sessions_helper(rserv, pserv, preset, temp, func, args=(), sessions=[]):
    ...
    if len(sessions) == 0:
        ...
        found_sessions = [
            s for s in sessions if os.path.exists(os.path.join(rserv, s))]
    ...
    for session in tqdm.tqdm(found_sessions, ncols=100, desc='Sessions'):
        ...
        r_server_session = os.path.normpath(
            os.path.join(rserv, session))
        p_server_session = os.path.normpath(
            os.path.join(pserv, session))

        if not os.path.exists(r_server_session):
            ws('Session {} does not exist on the server.'.format(
                r_server_session))
            continue

        if not os.path.exists(p_server_session):
            rs('Creating processed server session directory {}'.format(
                p_server_session))
