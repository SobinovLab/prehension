...
# -*- coding: utf-8 -*-
"""
Plot peri-event time histograms (PETH) for a session directly from the sorter
output (Phy export / SpikeInterface sorter folder) and the Open Ephys events,
without the NWB.  Same behaviour as neural_plotting.figure_peth, but the neural
inputs come from the sorter output rather than neural.nwb.
...
from prehension.neural_plotting.figure_peth2 import plot_perievent_histograms

if __name__ == "__main__":
    current_preset_name, current_preset, argv = preset.process_args_for_preset()

    parser = argparse.ArgumentParser(
        description="PETH traces from the sorter output (no NWB), coloured by an object property.")
